...
use regex_automata::{
    ...
    Anchored, Match, MatchKind, PatternSet, Span,
};
...
impl Builder {
    ...
    pub fn new() -> Builder {
        Builder { config: Config::default(), thompson: thompson::Compiler::new() }
    }
    ...
    pub fn build_many<P: AsRef<str>>(&self, patterns: &[P]) -> Result<PikeVM, BuildError> {
        let nfa = self.thompson.build_many(patterns).unwrap();
...
impl PikeVM {
    ...
    pub fn is_match<'h, I: Into<Input<'h>>>(&self, cache: &mut Cache, input: I) -> bool {
        ...
        self.search_slots(cache, &mut input, &mut []).is_some()
    }
    ...
    pub fn find<'h, I: Into<Input<'h>>>(&self, cache: &mut Cache, input: I) -> Option<Match> {
        ...
        if self.get_nfa().pattern_len() == 1 {
            ...
            let pid = self.search_slots(cache, &mut input, &mut slots)?;
    ...
    pub fn captures<'h, I: Into<Input<'h>>>(
        ...
        self.search(cache, &mut input.into(), caps)
    }
    ...
    pub fn captures_iter<'r, 'c, 'h, I: Into<Input<'h>>>(
        ...
        let it = iter::Searcher::new(input.into());
        CapturesMatches { re: vm, cache, caps, it }
    }
    ...
    /// to return `false`.
    ///
    /// This is like [`PikeVM::captures`], but it accepts a concrete `&mut Input`
    ...
    /// re.search(&mut cache, &mut Input::new(haystack), &mut caps);
    /// assert_eq!(expected, caps.get_match());
    ///
    ...
    /// re.search(&mut cache, &mut Input::new(&haystack[3..6]), &mut caps);
    /// assert_eq!(expected, caps.get_match());
    ///
    ...
    /// ```
    #[inline]
    pub fn search(&self, cache: &mut Cache, input: &mut Input<'_>, caps: &mut Captures) {
    ...
    fn search_imp(
        ...
        let mut prev_haystack: &[u8] = &[];
        let mut haystack_pos;
        macro_rules! move_to {
            ($at:expr) => {{
                let old_offset = input.haystack_off();
                haystack_pos = input.move_to($at).unwrap_or_default();
                if old_offset != input.haystack_off() && input.haystack_off() != 0 {
                    prev_haystack = input.advance_rev().unwrap();
                    input.advance_fwd().unwrap();
                }
            }};
        }
        move_to!(input.start());
        ...
        assert!(input.end() < core::usize::MAX, "byte slice lengths must be less than usize MAX",);
        instrument!(|c| c.reset(&vm.nfa));
        ...
        let allmatches = self.get_config().get_match_kind() == MatchKind::All;
        ...
        macro_rules! at {
            () => {
                input.haystack_off() + haystack_pos
            };
        }
        while at!() <= input.end() {
            if haystack_pos >= input.haystack().len() {
                debug_assert_eq!(haystack_pos, input.haystack().len());
                prev_haystack = input.haystack();
                if input.advance_fwd().is_some() {
                    haystack_pos = 0;
                }
            }
            ...
            if let Some(x) =
                self.nexts(stack, curr, next, input, haystack_pos, prev_haystack, slots)
            {
                pid = Some(x);
            }
        ...
        instrument!(|c| c.eprint(&vm.nfa));
        pid
    }

    // c    /// The implementation for the 'which_overlapping_matches' API. Basically,
    ...
    //     let allmatches = self.config.get_match_kind() == MatchKind::All;
    ...
    //         if !any_matches || allmatches {
    //             let slots = &mut [];
    //             self.epsilon_closure(stack, slots, curr, input, prev_chunk, at, start_id);
    //         }
    ...
    fn nexts(
        ...
        for sid in set.iter() {
            pid = match self.next(stack, slot_table, next, input, prev_haystack, haystack_pos, sid)
            {
                None => continue,
                Some(pid) => Some(pid),
            };
            slots.copy_from_slice(slot_table.for_state(sid));
            if self.config.get_match_kind() != MatchKind::All {
                break;
            }
        }
        pid
    ...
    fn nexts_overlapping(
        ...
        let ActiveStates { ref set, ref mut slot_table } = *curr;
        for sid in set.iter() {
            let pid = match self.next(stack, slot_table, next, input, prev_haystack, at, sid) {
                None => continue,
                Some(pid) => pid,
            };
    ...
    fn next(
        ...
        match *self.nfa.state(sid) {
            ...
            State::Sparse(ref sparse) => {
                if let Some(next_sid) = sparse.matches(input.haystack(), haystack_pos) {
                    let slots = curr_slot_table.for_state(sid);
                    // OK because 'at <= haystack.len() < usize::MAX', so
                    // adding 1 will never wrap.
                    let haystack_pos = haystack_pos.wrapping_add(1);
            ...
            State::Dense(ref dense) => {
                if let Some(next_sid) = dense.matches(input.haystack(), haystack_pos) {
                    let slots = curr_slot_table.for_state(sid);
                    // OK because 'at <= haystack.len() < usize::MAX', so
                    // adding 1 will never wrap.
                    let haystack_pos = haystack_pos.wrapping_add(1);
        ...
        loop {
            ...
            match *self.nfa.state(sid) {
                ...
                State::Look { look, next } => {
                    ...
                    // unavailable.
                    if !look::LookMatcher::new()
                        .set_line_terminator(self.nfa.look_matcher().get_line_terminator())
                        .matches_inline(look, prev_haystack, input, at)
                    {
                        return;
                    }
                    sid = next;
                ...
                State::Capture { next, slot, .. } => {
                    ...
                    if slot.as_usize() < curr_slots.len() {
                        instrument!(|c| c.record_stack_push(sid));
                        stack
                            .push(FollowEpsilon::RestoreCapture { slot, offset: curr_slots[slot] });
                        // OK because length of a slice must fit into an isize.
                        curr_slots[slot] =
                            Some(NonMaxUsize::new(input.haystack_off() + at).unwrap());
                    }
                    sid = next;
                }
            }
    ...
    fn start_config(&self, input: &mut Input<'_>) -> Option<(bool, StateID)> {
        match input.get_anchored() {
            // Only way we're unanchored is if both the caller asked for an
            // unanchored search *and* the pattern is itself not anchored.
            Anchored::No => {
                Some((self.get_nfa().is_always_start_anchored(), self.get_nfa().start_anchored()))
            }
            Anchored::Yes => Some((true, self.get_nfa().start_anchored())),
            Anchored::Pattern(pid) => Some((true, self.get_nfa().start_pattern(pid)?)),
        }
    }
}
...
impl Counters {
    ...
    fn reset(&mut self, nfa: &NFA) {
        ...
        vm.steps.clear();
        vm.steps.resize(len, 0);

        vm.closures.clear();
        vm.closures.resize(len, 0);

        vm.stack_pushes.clear();
        vm.stack_pushes.resize(len, 0);

        vm.set_inserts.clear();
        vm.set_inserts.resize(len, 0);
    }
    ...
    fn eprint(&self, nfa: &NFA) {
        ...
        let mut set_counts = vm.state_sets.iter().collect::<Vec<(&Vec<StateID>, &u64)>>();
    ...
    fn record_state_set(&mut self, set: &SparseSet) {
        ...
        *vm.state_sets.entry(set).or_insert(0) += 1;
    }
    ...
    fn record_stack_push(&mut self, sid: StateID) {
        vm.stack_pushes[sid] += 1;
    }

    fn record_set_insert(&mut self, sid: StateID) {
        vm.set_inserts[sid] += 1;
    }
}
