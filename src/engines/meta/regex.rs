...
use regex_automata::{
    ...
    HalfMatch, Match, MatchKind, PatternID, Span,
};
use regex_syntax::{
    ast,
    hir::{self, Hir},
...
impl Regex {
    ...
    pub fn new_many<P: AsRef<str>>(patterns: &[P]) -> Result<Regex, BuildError> {
        Self::builder().build_many(patterns)
    }
...
impl Regex {
    ...
    /// ```
    #[inline]
    pub fn is_match<C: Cursor>(&self, mut input: Input<C>) -> bool {
        input.earliest(true);
        if self.imp.info.is_impossible(&input) {
            return false;
        }
        let mut guard = self.pool.get();
        let result = self.imp.strat.is_match(&mut guard, &mut input);
    ...
    /// ```
    #[inline]
    pub fn find<C>(&self, input: Input<C>) -> Option<Match>
    where
        C: Cursor,
    {
        self.search(input)
    ...
    /// ```
    #[inline]
    pub fn captures<C: Cursor>(&self, input: Input<C>, caps: &mut Captures) {
        self.search_captures(input, caps)
    }
    ...
    /// ```
    #[inline]
    pub fn find_iter<C: Cursor>(&self, input: Input<C>) -> FindMatches<'_, C> {
    ...
    /// ```
    #[inline]
    pub fn captures_iter<C: Cursor>(&self, input: Input<C>) -> CapturesMatches<'_, C> {
        let cache = self.pool.get();
    ...
    /// ```
    #[inline]
    pub fn split<C: Cursor>(&self, input: Input<C>) -> Split<'_, C> {
    ...
    /// # Ok::<(), Box<dyn std::error::Error>>(())
    /// ```
    pub fn splitn<C: Cursor>(&self, input: Input<C>, limit: usize) -> SplitN<'_, C> {
...
impl Regex {
    ...
    /// ```
    #[inline]
    pub fn search<C: Cursor>(&self, mut input: Input<C>) -> Option<Match> {
        if self.imp.info.is_impossible(&input) {
            return None;
        }
        let mut guard = self.pool.get();
        let result = self.imp.strat.search(&mut guard, &mut input);
    ...
    /// ```
    #[inline]
    pub fn search_half<C: Cursor>(&self, mut input: Input<C>) -> Option<HalfMatch> {
        if self.imp.info.is_impossible(&input) {
            return None;
        }
        let mut guard = self.pool.get();
        let result = self.imp.strat.search_half(&mut guard, &mut input);
        // See 'Regex::search' for why we put the guard back explicitly.
        PoolGuard::put(guard);
        result
    }
    ...
    /// ```
    #[inline]
    pub fn search_captures<C: Cursor>(&self, input: Input<C>, caps: &mut Captures) {
    ...
    pub fn search_slots<C: Cursor>(
        ...
        let mut guard = self.pool.get();
        let result = self.imp.strat.search_slots(&mut guard, &mut input, slots);
        // See 'Regex::search' for why we put the guard back explicitly.
        PoolGuard::put(guard);
        result
    }
    ...
    // /// ```
    // #[inline]
    // pub fn which_overlapping_matches<C: Cursor>(&self, mut input: Input<C>, patset: &mut PatternSet) {
    //     if self.imp.info.is_impossible(input) {
    //         return;
    //     }
    //     let mut guard = self.pool.get();
...
impl Regex {
    ...
    /// ```
    #[inline]
    pub fn search_with<C: Cursor>(&self, cache: &mut Cache, input: &mut Input<C>) -> Option<Match> {
        if self.imp.info.is_impossible(input) {
            return None;
        }
    ...
    /// ```
    #[inline]
    pub fn search_half_with<C: Cursor>(
        &self,
        cache: &mut Cache,
        input: &mut Input<C>,
    ) -> Option<HalfMatch> {
    ...
    /// ```
    #[inline]
    pub fn search_captures_with<C: Cursor>(
        &self,
        cache: &mut Cache,
        input: &mut Input<C>,
        caps: &mut Captures,
    ...
    /// ```
    #[inline]
    pub fn search_slots_with<C: Cursor>(
        &self,
        cache: &mut Cache,
        input: &mut Input<C>,
        slots: &mut [Option<NonMaxUsize>],
...
impl RegexInfo {
    ...
    pub(crate) fn is_anchored_start(&self, input: &Input<impl Cursor>) -> bool {
    ...
    #[cfg_attr(feature = "perf-inline", inline(always))]
    fn is_impossible<C: Cursor>(&self, input: &Input<C>) -> bool {
...
impl<'r, C: Cursor> FindMatches<'r, C> {
...
impl<'r, C: Cursor> core::iter::FusedIterator for FindMatches<'r, C> {}
...
#[derive(Debug)]
pub struct CapturesMatches<'r, C: Cursor> {
    re: &'r Regex,
    cache: CachePoolGuard<'r>,
    caps: Captures,
    it: iter::Searcher<C>,
...
impl<'r, C: Cursor> CapturesMatches<'r, C> {
    /// Returns the `Regex` value that created this iterator.
    #[inline]
    pub fn regex(&self) -> &'r Regex {
        self.re
...
impl<'r, C: Cursor> Iterator for CapturesMatches<'r, C> {
    type Item = Captures;
...
impl<'r, C: Cursor> core::iter::FusedIterator for CapturesMatches<'r, C> {}
...
#[derive(Debug)]
pub struct Split<'r, C: Cursor> {
    finder: FindMatches<'r, C>,
    last: usize,
}

impl<'r, C: Cursor> Split<'r, C> {
    /// Returns the current `Input` associated with this iterator.
    ///
    /// The `start` position on the given `Input` may change during iteration,
    /// but all other values are guaranteed to remain invariant.
...
impl<'r, C: Cursor> Iterator for Split<'r, C> {
    type Item = Span;

    fn next(&mut self) -> Option<Span> {
        match self.finder.next() {
            None => {
                let len = self.finder.it.input().end();
                if self.last > len {
                    None
                } else {
...
impl<'r, C: Cursor> core::iter::FusedIterator for Split<'r, C> {}
...
#[derive(Debug)]
pub struct SplitN<'r, C: Cursor> {
    splits: Split<'r, C>,
    limit: usize,
}

impl<'r, C: Cursor> SplitN<'r, C> {
    /// Returns the current `Input` associated with this iterator.
    ///
    /// The `start` position on the given `Input` may change during iteration,
    /// but all other values are guaranteed to remain invariant.
...
impl<'r, C: Cursor> Iterator for SplitN<'r, C> {
    type Item = Span;
    ...
    fn next(&mut self) -> Option<Span> {
        ...
        let len = self.splits.finder.it.input().end();
...
impl<'r, C: Cursor> core::iter::FusedIterator for SplitN<'r, C> {}
...
impl Config {
    ...
    //     self.onepass_size_limit.unwrap_or(Some((1 << 20)))
    // }
...
impl Builder {
    ...
    pub fn build_many<P: AsRef<str>>(&self, patterns: &[P]) -> Result<Regex, BuildError> {
        ...
        for (pid, p) in patterns.iter().with_pattern_ids() {
            let ast =
                self.ast.build().parse(p.as_ref()).map_err(|err| BuildError::ast(pid, err))?;
            asts.push(ast);
        }
    ...
    /// Ok::<(), Box<dyn std::error::Error>>(())
    /// ```
    pub fn build_from_hir<C: Cursor>(&self, hir: &Hir) -> Result<Regex, BuildError> {
    ...
    pub fn syntax(&mut self, config: regex_automata::util::syntax::Config) -> &mut Builder {
        self.ast
            .ignore_whitespace(config.get_ignore_whitespace())
            .nest_limit(config.get_nest_limit())
            .octal(config.get_octal());
        self.hir
            .unicode(config.get_unicode())
            .case_insensitive(config.get_case_insensitive())
            .multi_line(config.get_multi_line())
            .crlf(config.get_crlf())
            .dot_matches_new_line(config.get_dot_matches_new_line())
            .line_terminator(config.get_line_terminator())
            .swap_greed(config.get_swap_greed())
